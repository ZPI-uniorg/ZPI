  ...
  const assignColumns = (dayEvents, dateStr) => {
    ...
    sortedEvents.forEach(event => {
      ...
      for (let i = 0; i < columns.length; i++) {
        ...
        const hasOverlap = columnEvents.some(ev => eventsOverlap(event, ev, dateStr));
    ...
    columns.forEach((columnEvents, colIndex) => {
      columnEvents.forEach(event => {
        ...
        const overlappingEvents = sortedEvents.filter(ev => 
          ev.id !== event.id && eventsOverlap(event, ev, dateStr)
        );
    ...
    <div className="flex-1 bg-slate-900/95 rounded-2xl shadow-[0_30px_60px_rgba(15,23,42,0.45)] border border-slate-700 p-4 overflow-hidden flex flex-col">
      ...
      <div className="grid grid-cols-[60px_repeat(7,1fr)] gap-px bg-slate-700/30 min-h-[40px] mb-1">
        ...
        {weekDays.map((d, dayIdx) => {
          ...
          return (
            <div key={dayIdx} className="bg-slate-900/95 p-1 flex flex-col gap-1">
              {allDayEvents.map((ev) => {
                ...
                return (
                  ...
                  >
                    {dayEvents.map((ev, evIdx) => {
                      ...
                      const columnInfo = eventColumns.get(ev.id) || { column: 0, totalColumns: 1, visibleColumns: 1 };
                      ...
                      const actualWidth = columnInfo.totalColumns > 1 
                        ? `calc(${width} - 4px)` 
                        : `calc(${width} - 2px)`;
