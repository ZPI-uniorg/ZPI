...
class HttpServerProxy {
    ...
    async sendHttpRequest(request, options, abortSignal) {
        ...
        function httpInvoke(url, request, abortSignal) {
            ...
            return new Promise((resolve, reject) => {
                const req = http_1.default.request(url, {
                    ...
                    res.on("end", () => {
                        logger_1.logger.info(`Received proxied response for '${getDisplayUrl(url)}: status code:${res.statusCode ?? 0}'`);
        ...
        logger_1.logger.info(`Received request from: '${request.HttpMethod} ${request.Url} , content-length: ${request.Content?.byteLength ?? 0}`);
